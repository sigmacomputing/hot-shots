...
function overrideTags (parent, child, telegraf) {
  ...
  const res = toAppend.length > 0 ? tags.concat(toAppend) : tags;
  return res.join(',');
}

function overrideTags2 (globalTags, globalTagsFullMemo, child, telegraf) {
  let result = '';
  ...
  for (const c of Object.keys(child)) {
    ...
    result += `${formattedChildKey}:${sanitizeTags(child[c], telegraf)}`;
  }
  if (usedGlobalKeys.size === 0) {
    result += globalTagsFullMemo;
  }
  else {
    for (const g of Object.keys(globalTags)) {
      if (!usedGlobalKeys.has(g)) {
        const xy = `${g}:${sanitizeTags(globalTags[g], telegraf)}`;
        result = xy + result;
      }
    }
  }
