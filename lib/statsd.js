...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  this.globalTagsOnlyStringJoinMemo = {};
  if (typeof options.globalTags === 'object') {
    const formatted = helpers.formatTags({}, '', options.globalTags, options.telegraf);
    this.globalTagsOnlyStringJoinMemo
  }
...
Client.prototype.send = function (message, tags, callback) {
  ...
  if (mergedTags.length > 0) {
    if (this.telegraf) {
      ...
      message = `${messageSplit[0]},${mergedTags.join(',').replace(/:/g, '=')}:${messageSplit.slice(1).join(':')}`;
    } else {
