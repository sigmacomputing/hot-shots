...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  this.globalTagsFormattedKeyToFinalValue = {};
  if (typeof options.globalTags === 'object') {
   for (const unformattedKey of Object.keys(options.globalTags)) {
     const formattedKey = helpers.sanitizeTags(unformattedKey, telegraf);
     const fullyFormatedResult = helpers.formatTag(unformattedKey, options.globalTags[unformattedKey], telegraf);
     this.globalTagsFormattedKeyToFinalValue[formattedKey] = fullyFormatedResult;
   }
  }
  const globalTagsForStringMemo = Object.values(this.globalTagsFormattedKeyToFinalValue);
  this.globalTagsOnlyStringJoinMemo = globalTagsForStringMemo.length > 0 ?  `|${this.tagPrefix}${globalTagsForStringMemo.join(this.tagSeparator)}` : '';
...
Client.prototype.send = function (message, tags, callback) {
  ...
  if (tags && typeof tags === 'object') {
    if (this.globalTagsFormattedKeyToFinalValue && !Array.isArray(tags)) {
      // short-circuit
      if (!this.telegraf && Object.keys(tags).length === 0) {
        message += this.globalTagsOnlyStringJoinMemo;
        this._send(message, callback);
        return;
      }
      mergedTags = helpers.overrideTags2(this.globalTagsFormattedKeyToFinalValue, tags, this.telegraf);
    }
    else {
